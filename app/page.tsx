...
import { AlertCircle, ArrowRightLeft, BarChart3, BellRing, Building2, ChevronRight, CircleDollarSign, Clock3, Gauge, Globe2, LayoutDashboard, LoaderCircle, LockKeyhole, Menu, RefreshCw, Search, ShieldAlert, ShieldCheck, Star, TrendingDown, WalletCards, X } from 'lucide-react';
...
import { SlTpSettings } from '@/components/sl-tp-settings';
import { TrailingStopSettings } from '@/components/trailing-stop-settings';
import { DeadCrossSettings } from '@/components/dead-cross-settings';
import { StrategySettingsSync } from '@/components/strategy-server-sync';
import { StrategySettingsTransfer } from '@/components/strategy-settings-transfer';
...
const settingItems = [
  ...
  { id: 'sl-tp', label: 'SL / TP', icon: ShieldAlert },
  { id: 'trailing-stop', label: '트레일링 스탑', icon: Gauge },
  { id: 'dead-cross', label: '데드크로스', icon: TrendingDown },
  { id: 'strategy-transfer', label: '전략 설정 옮기기', icon: ArrowRightLeft },
] as const;
...
function Dashboard() {
  ...
  const [feature, setFeature] = useState<'account' | 'search' | 'rankings' | 'watchlist' | 'sl-tp' | 'trailing-stop' | 'dead-cross' | 'strategy-transfer' | 'notifications'>('account');
  ...
  return <main className="min-h-screen bg-slate-50 text-slate-950">
    ...
    <div className="flex min-h-[calc(100vh-65px)]">
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 border-r border-slate-200 bg-white px-3 py-5 transition-transform lg:sticky lg:top-16 lg:z-10 lg:h-[calc(100vh-65px)] lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav aria-label="주요 기능" className="space-y-1">{menuItems.map((item) => { const Icon = item.icon; const active = item.id === feature; return <button key={item.label} type="button" onClick={() => { setFeature(item.id); setSidebarOpen(false); }} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium ${active ? 'bg-emerald-50 text-emerald-900' : 'text-slate-600 hover:bg-slate-50'}`}><Icon className="size-4"/>{item.label}{active && <ChevronRight className="ml-auto size-4"/>}</button>; })}</nav>
        <p className="mt-6 px-3 pb-2 text-[10px] font-bold uppercase tracking-[0.16em] text-slate-400">Setting</p>
        <nav aria-label="설정" className="space-y-1">{settingItems.map((item) => { const Icon = item.icon; const active = item.id === feature; return <button key={item.label} type="button" onClick={() => { setFeature(item.id); setSidebarOpen(false); }} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium ${active ? 'bg-emerald-50 text-emerald-900' : 'text-slate-600 hover:bg-slate-50'}`}><Icon className="size-4"/>{item.label}{active && <ChevronRight className="ml-auto size-4"/>}</button>; })}</nav>
      ...
      <section className="min-w-0 flex-1 px-4 py-6 md:px-8 md:py-8 xl:px-10"><div className="mx-auto max-w-[1440px]">
        ...
        </section> : feature === 'rankings' ? <RankingsView key={environment} environment={environment} onEnvironmentChange={changeEnvironment} onSelect={(stock) => setSelectedTrade({ stock, mode: 'buy' })}/> : feature === 'watchlist' ? <WatchlistView onSelect={(stock) => setSelectedTrade({ stock, mode: 'buy' })}/> : feature === 'sl-tp' ? <SlTpSettings environment={environment} onEnvironmentChange={changeEnvironment}/> : feature === 'trailing-stop' ? <TrailingStopSettings environment={environment} onEnvironmentChange={changeEnvironment}/> : feature === 'dead-cross' ? <DeadCrossSettings environment={environment} onEnvironmentChange={changeEnvironment}/> : feature === 'strategy-transfer' ? <StrategySettingsTransfer/> : <NotificationSettings/>}
      </div></section>
    </div>
...
export default function Home() { return <QueryProvider><WatchlistProvider><Dashboard/></WatchlistProvider></QueryProvider>; }
