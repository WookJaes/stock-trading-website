...
import { AlertCircle, BarChart3, BellRing, Building2, ChevronRight, CircleDollarSign, Clock3, Gauge, Globe2, LayoutDashboard, LoaderCircle, LockKeyhole, Menu, RefreshCw, Search, ShieldAlert, ShieldCheck, Star, TrendingDown, WalletCards, X } from 'lucide-react';
...
function Dashboard() {
  ...
  const [feature, setFeature] = useState<'account' | 'search' | 'rankings' | 'watchlist' | 'sl-tp' | 'trailing-stop' | 'dead-cross' | 'notifications'>('account');
  ...
  return <main className="min-h-screen bg-slate-50 text-slate-950">
    ...
    <div className="flex min-h-[calc(100vh-65px)]">
      ...
      <section className="min-w-0 flex-1 px-4 py-6 md:px-8 md:py-8 xl:px-10"><div className="mx-auto max-w-[1440px]">
        ...
        </section> : feature === 'rankings' ? <RankingsView key={environment} environment={environment} onEnvironmentChange={changeEnvironment} onSelect={(stock) => setSelectedTrade({ stock, mode: 'buy' })}/> : feature === 'watchlist' ? <WatchlistView onSelect={(stock) => setSelectedTrade({ stock, mode: 'buy' })}/> : feature === 'sl-tp' ? <SlTpSettings environment={environment} onEnvironmentChange={changeEnvironment}/> : feature === 'trailing-stop' ? <TrailingStopSettings environment={environment} onEnvironmentChange={changeEnvironment}/> : feature === 'dead-cross' ? <DeadCrossSettings environment={environment} onEnvironmentChange={changeEnvironment}/> : <NotificationSettings/>}
      </div></section>
    </div>
