        ...
        {query.isPending ? <Loading/> : query.isError ? <div className="rounded-2xl border border-amber-200 bg-amber-50 p-6"><div className="flex gap-3"><AlertCircle className="mt-0.5 size-5 shrink-0 text-amber-700"/><div><h2 className="font-semibold text-amber-950">계좌 정보를 연결하지 못했습니다</h2><p className="mt-1 text-sm text-amber-800">{query.error.message}</p><button type="button" onClick={() => query.refetch()} className="mt-4 rounded-lg bg-amber-900 px-3 py-2 text-xs font-semibold text-white">다시 시도</button></div></div></div> : data ? <>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_1px_2px_rgba(15,23,42,0.03)] sm:p-5">
            <div className="mb-4 flex gap-2" aria-label="검색 시장">
              <button type="button" disabled={environment !== 'domestic-mock'} className={`rounded-lg px-4 py-2 text-sm font-semibold ${environment === 'domestic-mock' ? 'bg-emerald-900 text-white' : 'cursor-not-allowed bg-slate-100 text-slate-400'}`}>국내</button>
              <button type="button" disabled={environment !== 'overseas-mock'} className={`rounded-lg px-4 py-2 text-sm font-semibold ${environment === 'overseas-mock' ? 'bg-emerald-900 text-white' : 'cursor-not-allowed bg-slate-100 text-slate-400'}`}>해외</button>
            </div>
            <form onSubmit={(event) => { event.preventDefault(); setSearchTerm(searchInput.trim()); }} className="flex flex-col gap-2 sm:flex-row">
              <label className="relative flex-1"><span className="sr-only">종목명 또는 종목코드</span><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400"/><input value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder={environment === 'domestic-mock' ? '예: 삼성전자, 005930' : '예: 테슬라, TSLA'} className="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 pl-10 pr-4 text-sm outline-none transition focus:border-emerald-700 focus:bg-white focus:ring-3 focus:ring-emerald-100"/></label>
