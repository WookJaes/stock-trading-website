...
export function RankingsView({ environment, onSelect }: { environment: Environment; onSelect: (stock: TradeStock) => void }) {
  ...
  const currency = environment === 'domestic-mock' ? 'KRW' : 'USD';
  ...
  return <section>
    ...
    <div className="mb-5 rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_1px_2px_rgba(15,23,42,0.03)]">
      <div className="flex gap-2" aria-label="순위 시장"><button type="button" disabled={environment !== 'domestic-mock'} className={`rounded-lg px-4 py-2 text-sm font-semibold ${environment === 'domestic-mock' ? 'bg-emerald-900 text-white' : 'cursor-not-allowed bg-slate-100 text-slate-400'}`}>국내</button><button type="button" disabled={environment !== 'overseas-mock'} className={`rounded-lg px-4 py-2 text-sm font-semibold ${environment === 'overseas-mock' ? 'bg-emerald-900 text-white' : 'cursor-not-allowed bg-slate-100 text-slate-400'}`}>해외</button></div>
    ...
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
      <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4"><div><h2 className="font-bold">{categories.find((item) => item.id === category)?.label}</h2><p className="mt-1 text-xs text-slate-500">종목을 선택하면 매수 패널이 열립니다.</p></div>{query.data && <span className="text-[11px] text-slate-400">{new Date(query.data.asOf).toLocaleString('ko-KR')}</span>}</div>
