...
import {
  ...
  type ExcludedStock,
  type TrailingStopMarketSettings,
  type StrategyMarket,
...
export function TrailingStopSettings({
  environment,
  onEnvironmentChange,
}: {
  environment: KiwoomEnvironment;
  ...
  const marketSettings = settings.strategies.trailingStop[market];
  const exportedText = serializeStrategySettings(settings);
  const query = useQuery({
    queryKey: ['trailing-stop-excluded-stock-search', environment, searchTerm],
  ...
  function updateMarket(
    updater: (
      current: TrailingStopMarketSettings,
    ...
    saveSettings({
      ...
      strategies: {
        ...settings.strategies,
        trailingStop: {
          ...settings.strategies.trailingStop,
          [market]: updater(settings.strategies.trailingStop[market]),
        },
      },
    });
  }
              ...
              <label className="text-sm font-bold text-slate-800">
                ...
                <div className="relative">
                  <input
                    ...
                    min="0.01"
                    step="0.01"
                    value={marketSettings.activationProfitPercent}
                    onChange={(event) =>
                      updateMarket((current) => ({
                        ...current,
                        activationProfitPercent: percentValue(
                          event.target.value,
                          current.activationProfitPercent,
                        ),
                      }))
                    }
              ...
              <label className="text-sm font-bold text-slate-800">
                ...
                <div className="relative">
                  <input
                    ...
                    min="0.01"
                    step="0.01"
                    value={marketSettings.drawdownPercent}
                    onChange={(event) =>
                      updateMarket((current) => ({
                        ...current,
                        drawdownPercent: percentValue(
                          event.target.value,
