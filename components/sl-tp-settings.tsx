...
function percentValue(value: string, fallback: number) {
  const parsed = Number(value);
  return Number.isFinite(parsed) && parsed > 0 ? parsed : fallback;
}

export function SlTpSettings({
  environment,
  onEnvironmentChange,
}: {
  environment: KiwoomEnvironment;
  ...
  const marketSettings = settings.strategies.slTp[market];
  const exportedText = serializeStrategySettings(settings);
  const query = useQuery({
    queryKey: ['strategy-excluded-stock-search', environment, searchTerm],
  ...
  function updateMarket(
    updater: (current: SlTpMarketSettings) => SlTpMarketSettings,
  ) {
    saveSettings({
      ...settings,
      strategies: {
        ...settings.strategies,
        slTp: {
          ...settings.strategies.slTp,
          [market]: updater(settings.strategies.slTp[market]),
        },
      },
    });
  }
  ...
  return (
    <section>
      ...
      <div className="mb-5 flex gap-3 rounded-xl border border-sky-200 bg-sky-50 px-4 py-3 text-sm text-sky-900">
        ...
        <p>
          평균 매입가를 기준으로 조건을 판단하고, 매도 가능 수량 전량을 시장가로
          매도하는 전략입니다. 서버가 신호 기록 전용이면 주문하지 않으며,
          모의 자동주문 활성 상태에서만 시장가 주문을 실행합니다.
        </p>
      </div>
              ...
              <label className="text-sm font-bold text-slate-800">
                ...
                <div className="relative">
                  <input
                    ...
                    min="0.01"
                    step="0.01"
                    value={marketSettings.takeProfitPercent}
                    onChange={(event) =>
                      updateMarket((current) => ({
                        ...current,
                        takeProfitPercent: percentValue(
                          event.target.value,
                  ...
                  />
                  <span className="absolute right-3 top-[22px] text-sm font-semibold text-rose-600">
                    %
                  </span>
                </div>
              ...
              <label className="text-sm font-bold text-slate-800">
                ...
                <div className="relative">
                  <input
                    ...
                    min="0.01"
                    step="0.01"
                    value={marketSettings.stopLossPercent}
                    onChange={(event) =>
                      updateMarket((current) => ({
                        ...current,
                        stopLossPercent: percentValue(
                          event.target.value,
                    ...
                    className={`${inputClass} pr-9`}
                  />
                  <span className="absolute right-3 top-[22px] text-sm font-semibold text-blue-600">
                    %
                  </span>
                </div>
                <span className="mt-1.5 block text-xs font-normal text-slate-500">
