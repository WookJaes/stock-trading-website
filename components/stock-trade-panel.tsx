...
export function StockTradePanel({ stock, environment, onClose }: { stock: TradeStock; environment: Environment; onClose: () => void }) {
  ...
  const statusParams = order ? new URLSearchParams({ environment, action: 'status', code: stock.code, market: stock.market, orderNo: order.orderNo }) : null;
  const statusQuery = useQuery({ queryKey: ['order-status', environment, order?.orderNo], queryFn: () => json<OrderStatus>(fetch(`/api/trade?${statusParams}`)), enabled: Boolean(order), refetchInterval: (query) => query.state.data?.status === 'filled' ? false : 5_000 });
  const orderMutation = useMutation({
    mutationFn: (idempotencyKey: string) => json<OrderResponse>(fetch('/api/trade', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ environment, code: stock.code, market: stock.market, quantity: Number(quantity), orderType, price: orderType === 'limit' ? Number(price) : 0, idempotencyKey }) })),
  ...
  const valid = Number.isInteger(Number(quantity)) && Number(quantity) > 0 && (orderType === 'market' || Number(price) > 0);
  ...
  return <><button type="button" className="fixed inset-0 z-40 bg-slate-950/35 backdrop-blur-[2px]" onClick={onClose} aria-label="거래 패널 닫기"/><dialog open aria-label={`${stock.name} 거래 패널`} className="fixed inset-y-0 right-0 z-50 m-0 ml-auto flex h-full w-full max-w-lg flex-col overflow-y-auto border-0 border-l border-slate-200 bg-slate-50 p-0 shadow-2xl">
    <header className="sticky top-0 z-10 flex items-start justify-between border-b border-slate-200 bg-white/95 px-5 py-4 backdrop-blur"><div><p className="text-xs font-semibold text-emerald-800">모의투자 종목</p><h2 className="mt-1 text-xl font-bold">{stock.name}</h2><p className="mt-1 text-xs text-slate-500">{stock.code} · {stock.market}</p></div><button type="button" onClick={onClose} className="grid size-9 place-items-center rounded-lg hover:bg-slate-100" aria-label="닫기"><X className="size-5"/></button></header>
    <div className="space-y-4 p-4 sm:p-5">
      ...
      {detailQuery.isPending ? <div className="grid min-h-48 place-items-center"><LoaderCircle className="size-6 animate-spin text-emerald-800"/></div> : detailQuery.isError ? <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">{detailQuery.error.message}</div> : detail && <>
        ...
        {!detail.tradable ? <div className="flex gap-2 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><AlertCircle className="size-4 shrink-0"/><p>{detail.unavailableReason}</p></div> : <section className="rounded-2xl border border-slate-200 bg-white p-5"><div className="flex items-center justify-between"><h3 className="font-bold">매수 주문</h3><span className="inline-flex items-center gap-1 text-xs font-semibold text-emerald-700"><ShieldCheck className="size-4"/>모의투자 전용</span></div>
          ...
          <label className="mt-4 block text-xs font-semibold text-slate-600">주문수량<input type="number" min="1" step="1" value={quantity} onChange={(event) => setQuantity(event.target.value)} className="mt-2 h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm outline-none focus:border-emerald-700"/></label>
          ...
          <button type="button" disabled={!valid || orderMutation.isPending || Boolean(order)} onClick={() => setConfirming(true)} className="mt-5 h-11 w-full rounded-xl bg-emerald-900 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-50">{order ? '주문 접수 완료' : '매수 주문 확인'}</button>
        </section>}
        {order && <section className="rounded-2xl border border-slate-200 bg-white p-5"><div className="flex items-center justify-between"><h3 className="font-bold">주문 상태</h3><button type="button" onClick={() => statusQuery.refetch()} disabled={statusQuery.isFetching} className="grid size-8 place-items-center rounded-lg border border-slate-200"><RefreshCw className={`size-4 ${statusQuery.isFetching ? 'animate-spin' : ''}`}/></button></div><p className="mt-3 text-sm font-semibold text-slate-700">{statusQuery.data?.statusLabel ?? '체결 확인 중'}</p>{statusQuery.data?.orderedQuantity !== undefined && <dl className="mt-3 grid grid-cols-3 gap-2 text-xs"><div><dt className="text-slate-400">주문</dt><dd className="mt-1 font-semibold">{statusQuery.data.orderedQuantity}주</dd></div><div><dt className="text-slate-400">체결</dt><dd className="mt-1 font-semibold">{statusQuery.data.filledQuantity}주</dd></div><div><dt className="text-slate-400">미체결</dt><dd className="mt-1 font-semibold">{statusQuery.data.remainingQuantity}주</dd></div></dl>}{statusQuery.data?.rejectedReason && <p className="mt-3 text-xs text-rose-600">{statusQuery.data.rejectedReason}</p>}</section>}
      </>}
    </div>
  </dialog>
  {confirming && <div className="fixed inset-0 z-[60] grid place-items-center bg-slate-950/45 p-4"><dialog open aria-label="매수 주문 최종 확인" className="relative m-0 w-full max-w-sm rounded-2xl bg-white p-5 shadow-2xl"><h3 className="text-lg font-bold">매수 주문을 접수할까요?</h3><p className="mt-2 text-sm leading-relaxed text-slate-600">{stock.name} {quantity}주를 {orderType === 'market' ? '시장가' : `${price} ${detail?.currency} 지정가`}로 주문합니다. 모의투자 주문이 즉시 전송됩니다.</p><div className="mt-5 grid grid-cols-2 gap-2"><button type="button" onClick={() => setConfirming(false)} className="h-10 rounded-xl border border-slate-200 text-sm font-semibold">취소</button><button type="button" onClick={submitOrder} disabled={orderMutation.isPending} className="h-10 rounded-xl bg-emerald-900 text-sm font-bold text-white disabled:opacity-60">{orderMutation.isPending ? '접수 중' : '최종 확인'}</button></div></dialog></div>}
  </>;
}
