...
현재 실투자 환경과 실투자 주문은 코드에서 계속 비활성화되어 있다. 이 배포 작업은 고정 IP 실행 기반만 제공하며 실투자 주문을 활성화하지 않는다.
...
- 현재 코드에 정의되지 않은 실투자 환경변수 이름을 임의로 만들지 않는다.
- 실투자 인증정보는 실투자 기능의 서버 측 설계와 보호 장치를 구현한 뒤 별도로 추가한다.
