...
- 전략 설정 옮기기 메뉴에서 전체 전략 설정 JSON 복사·가져오기
...
전략 설정을 다른 환경으로 옮기려면 로그인 후 `SETTING → 전략 설정 옮기기` 메뉴를 엽니다. 현재 설정을 `JSON 복사`로 복사한 뒤 다른 로컬 또는 Lightsail 사이트의 같은 메뉴에 붙여 넣고 `붙여넣은 전체 설정 적용`을 선택합니다. JSON에는 SL/TP·트레일링 스탑·데드크로스 설정과 제외종목만 포함되며, 비밀번호·앱 키·토큰·계좌정보·주문 실행 상태는 포함되지 않습니다. 활성화된 전략이 포함된 JSON은 적용 전에 확인이 필요합니다.
